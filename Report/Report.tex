...
When we draw our decision boundary, we want the boundary to be drawn as far away from each class. We call this, maximizing our margin, where the
distance from the first class is equally as far from the boundary as the second class. 
...
From this equation, we have our margin maximization and we have the error term, which is the sum of our hinge loss. C is our penalty term. If we increase C then our margin will get smaller if there are points that are misclassified and are correctly classified but inside the margin. 

\subsection{Citation and References List}
